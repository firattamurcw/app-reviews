...
from appstore_reviews.core.dedupe import dedupe_reviews, sort_reviews_newest_first
...
# ---------------------------------------------------------------------------
# dedupe_reviews — cross-provider: connect preferred over rss
# ---------------------------------------------------------------------------


class TestDedupeConnectPreferredOverRss:
    def test_connect_preferred_for_same_canonical_key(self) -> None:
        key = "app1::shared::r1"
        rss_review = _make_review(
            review_id="r1-rss",
            canonical_key=key,
            source_review_id="r1",
            source="rss",
        ...
        connect_review = _make_review(
            review_id="r1-connect",
            canonical_key=key,
            source_review_id="r1",
            source="connect",
            created_at=_dt(0),
        )
        result = dedupe_reviews([rss_review, connect_review])
        assert len(result) == 1
        assert result[0].source == "connect"

    def test_connect_preferred_regardless_of_order(self) -> None:
        key = "app1::shared::r1"
        connect_review = _make_review(
            review_id="r1-connect",
            canonical_key=key,
        ...
        # rss has newer created_at but connect should still win
        result = dedupe_reviews([connect_review, rss_review])
        assert len(result) == 1
        assert result[0].source == "connect"
...
class TestDedupeContentTimeHeuristic:
    ...
    def test_heuristic_connect_preferred_over_rss(self) -> None:
        a, b = self._make_pair(
            dt_offset_a=0, dt_offset_b=10, source_a="rss", source_b="connect"
        )
        result = dedupe_reviews([a, b])
        assert len(result) == 1
        assert result[0].source == "connect"
