...
class TestRssProvider:
    ...
    def test_multiple_countries_fetch_independently(self, mock_get: Any) -> None:
        ...
        for country in ["us", "gb", "jp"]:
            provider.fetch_page("12345", country, cursor=None)
    ...
    def test_pagination_fetches_multiple_pages(self, mock_get: Any) -> None:
        ...
        provider = RSSProvider()
        page1 = provider.fetch_page("12345", "us", cursor=None)
        assert page1.next_cursor is not None
        provider.fetch_page("12345", "us", cursor=page1.next_cursor)
    ...
    def test_partial_failure_across_countries(self, mock_get: Any) -> None:
        ...
        provider = RSSProvider()
        us_page = provider.fetch_page("12345", "us", cursor=None)
        gb_page = provider.fetch_page("12345", "gb", cursor=None)

        assert len(us_page.reviews) == 1
        assert us_page.error is None
        assert gb_page.reviews == []
        assert gb_page.error is not None
        assert gb_page.error.country == "gb"
...
class TestConnectProvider:
    ...
    def test_pagination_follows_next_link(self, mock_get: Any) -> None:
        ...
        provider = ConnectProvider("Bearer fake")
        page1 = provider.fetch_page("12345", "", cursor=None)
        assert page1.next_cursor is not None
        page2 = provider.fetch_page("12345", "", cursor=page1.next_cursor)

        assert call_count == 2
        assert len(page1.reviews) == 1
    ...
    def test_auth_header_is_sent(self, mock_get: Any) -> None:
        ...
        provider.fetch_page("12345", "", cursor=None)
