...
from app_reviews.tui.screens.app_id_input import AppIdInputScreen
from app_reviews.tui.screens.confirm_fetch import ConfirmFetchScreen
from app_reviews.tui.screens.country_select import CountrySelectScreen
from app_reviews.tui.screens.fetch_options import FetchOptions, FetchOptionsScreen
from app_reviews.tui.screens.fetching import FetchData, FetchingScreen
from app_reviews.tui.screens.review_browser import ReviewBrowserScreen
...
class ReviewApp(App[None]):
    ...
    def _on_app_id_entered(self, app_id: str | None) -> None:
        ...
        if not self._store and app_id:
            try:
                ...
                from app_reviews.tui.screens.store_select import (
                    StoreSelectScreen,
                )
    ...
    def _start_fetch(self, countries: list[str]) -> None:
        ...
        if self._app_id:
            config.app_ids = [self._app_id]
        config.countries = countries
        if self._store:
            config.store = self._store  # type: ignore[assignment]

        self.push_screen(
            FetchingScreen(config),
